...
def create_app():
    ...
    def get_output_language():
        ...
        try:
            ...
            return {
                "data": {"language": settings.output_language or Config.OUTPUT_LANGUAGE}
            }
...
def _load_settings_to_config(app):
    ...
    try:
        ...
        logging.info(
            f"Loaded reasoning config: text={settings.enable_text_reasoning}(budget={settings.text_thinking_budget}), image={settings.enable_image_reasoning}(budget={settings.image_thinking_budget})"
        )
        ...
        if settings.text_model_source:
            ...
            logging.info(
                f"Loaded TEXT_MODEL_SOURCE from settings: {settings.text_model_source}"
            )
        if settings.image_model_source:
            app.config["IMAGE_MODEL_SOURCE"] = settings.image_model_source
            logging.info(
                f"Loaded IMAGE_MODEL_SOURCE from settings: {settings.image_model_source}"
            )
        if settings.image_caption_model_source:
            app.config["IMAGE_CAPTION_MODEL_SOURCE"] = (
                settings.image_caption_model_source
            )
            logging.info(
                f"Loaded IMAGE_CAPTION_MODEL_SOURCE from settings: {settings.image_caption_model_source}"
            )
        ...
        if settings.lazyllm_api_keys:
            ...
            try:
                ...
                logging.info(
                    f"Loaded LazyLLM API keys for vendors: {[v for v, k in keys.items() if k and v.lower() in ALLOWED_LAZYLLM_VENDORS]}"
                )
        ...
        if isinstance(e, SQLAlchemyError) and "no such table: settings" in str(e):
            logging.debug(
                f"Settings table not yet created (expected on first boot): {e}"
            )
        else:
...
if __name__ == "__main__":
    ...
    if os.getenv("IN_DOCKER", "0") == "1":
        ...
        try:
            ...
            logging.info(f"Starting waitress production server on port {port}")
