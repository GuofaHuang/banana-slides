...
class ElementExtractor(ABC):
    ...
    @abstractmethod
    def extract(
        self, image_path: str, element_type: Optional[str] = None, **kwargs
    ) -> ExtractionResult:
        """
        从图像中提取元素
...
class MinerUElementExtractor(ElementExtractor):
    ...
    def _parse_image(
        ...
        try:
            ...
            mineru_result_dir = (
                self._upload_folder / "mineru_files" / extract_id
            ).resolve()
    ...
    def _extract_from_result(
        self, mineru_result_dir: str, target_image_size: Tuple[int, int], depth: int
    ) -> List[Dict[str, Any]]:
        """从MinerU结果目录中提取元素"""
        elements = []
                ...
                if block_type in ["header", "footer"]:
                    ...
                    if block.get("lines"):
                        for line in block["lines"]:
                            for span in line.get("spans", []):
                                if (
                                    span.get("type") in ["text", "inline_equation"]
...
def _shrink_cells_to_avoid_overlap(
    valid_cells: List[Dict], depth: int
) -> List[List[float]]:
    TARGET_MIN_GAP = 6
    SHRINK_STEP = 0.02
    MIN_SIZE_RATIO = 0.4
    ...
    while iteration < MAX_ITERATIONS:
        ...
        if current_min_gap >= TARGET_MIN_GAP:
            if iteration == 0:
                logger.info(f"{'  ' * depth}Cell gaps OK (min={current_min_gap:.1f}px)")
            else:
                logger.info(
                    f"{'  ' * depth}Shrink done: {iteration} iters, min gap={current_min_gap:.1f}px"
                )
            break
        ...
        if not all_cells_can_shrink:
            logger.warning(
                f"{'  ' * depth}Min size limit reached, current min gap={current_min_gap:.1f}px"
            )
            break
    ...
    if iteration >= MAX_ITERATIONS:
        current_min_gap = calculate_min_gap(cell_data)
        logger.warning(
            f"{'  ' * depth}Max iterations reached, current min gap={current_min_gap:.1f}px"
        )
...
class BaiduOCRElementExtractor(ElementExtractor):
    ...
    def _shrink_cells_to_avoid_overlap(
        self, valid_cells: List[Dict], depth: int
    ) -> List[List[float]]:
        return _shrink_cells_to_avoid_overlap(valid_cells, depth)
...
class PaddleOCRElementExtractor(ElementExtractor):
    """PaddleOCR-based element extractor for table recognition."""

    def __init__(self, paddle_table_ocr_provider):
        self._ocr_provider = paddle_table_ocr_provider
    ...
    def extract(
        ...
        try:
            ...
            logger.info(f"{'  ' * depth}PaddleOCR recognized {len(table_cells)} cells")

            body_cells = [cell for cell in table_cells if cell.get("section") == "body"]
            valid_cells = [cell for cell in body_cells if cell.get("text", "").strip()]

            if not valid_cells:
                logger.warning(f"{'  ' * depth}No valid table cells found")
                return ExtractionResult(elements=elements)

            cell_bboxes = []
            if shrink_cells:
                cell_bboxes = _shrink_cells_to_avoid_overlap(valid_cells, depth)
            else:
                cell_bboxes = [cell.get("bbox", [0, 0, 0, 0]) for cell in valid_cells]
            ...
            logger.info(
                f"{'  ' * depth}PaddleOCR extracted {len(elements)} cell elements"
            )

        except Exception as e:
            error_msg = f"PaddleOCR table recognition failed: {e}"
            logger.error(f"{'  ' * depth}{error_msg}", exc_info=True)
            return ExtractionResult(elements=elements, error=error_msg)
...
class PaddleAccurateOCRElementExtractor(ElementExtractor):
    """PaddleOCR-based element extractor for text recognition."""

    def __init__(self, paddle_ocr_provider):
        self._ocr_provider = paddle_ocr_provider
    ...
    def extract(
        ...
        try:
            ...
            text_lines = ocr_result.get("text_lines", [])
            logger.info(
                f"{'  ' * depth}PaddleOCR text recognized {len(text_lines)} lines"
            )

            for line_info in text_lines:
                text = line_info.get("text", "")
                bbox = line_info.get("bbox", [0, 0, 0, 0])
                probability = line_info.get("probability", None)
            ...
            logger.info(
                f"{'  ' * depth}PaddleOCR text extracted {len(elements)} elements"
            )

        except Exception as e:
            error_msg = f"PaddleOCR text recognition failed: {e}"
            logger.error(f"{'  ' * depth}{error_msg}", exc_info=True)
            return ExtractionResult(elements=elements, error=error_msg)
...
class ExtractorRegistry:
    ...
    def register(
        self, element_type: str, extractor: ElementExtractor
    ) -> "ExtractorRegistry":
        """
        注册元素类型到提取器的映射
    ...
    def register_types(
        self, element_types: List[str], extractor: ElementExtractor
    ) -> "ExtractorRegistry":
        """
        批量注册多个元素类型到同一个提取器
    ...
    def create_default(
        ...
        paddle_ocr_extractor: Optional[ElementExtractor] = None,
        paddle_accurate_ocr_extractor: Optional[ElementExtractor] = None,
    ) -> "ExtractorRegistry":
        """
        创建默认配置的注册表

        默认配置：
        - 表格类型 → PaddleOCR（优先）或百度表格OCR（如果可用）
        - 文字类型 → PaddleOCR（优先）或百度高精度OCR（如果可用），否则MinerU
        - 图片类型 → MinerU
        - 其他类型 → MinerU（默认）
        ...
        Args:
            ...
            paddle_ocr_extractor: PaddleOCR表格提取器实例（可选）
            paddle_accurate_ocr_extractor: PaddleOCR文字提取器实例（可选）
        ...
        if ocr_provider == "paddle" and paddle_ocr_extractor:
            table_extractor = paddle_ocr_extractor
        elif baidu_ocr_extractor:
            table_extractor = baidu_ocr_extractor
        else:
            table_extractor = mineru_extractor
        registry.register_types(list(cls.TABLE_TYPES), table_extractor)

        if ocr_provider == "paddle" and paddle_accurate_ocr_extractor:
            text_extractor = paddle_accurate_ocr_extractor
        elif baidu_accurate_ocr_extractor:
            text_extractor = baidu_accurate_ocr_extractor
        else:
            text_extractor = mineru_extractor
        registry.register_types(list(cls.TEXT_TYPES), text_extractor)

        logger.info(
            f"Created ExtractorRegistry: "
            f"table->{table_extractor.__class__.__name__}, "
            f"text->{text_extractor.__class__.__name__}, "
            f"image->{mineru_extractor.__class__.__name__}"
        )
