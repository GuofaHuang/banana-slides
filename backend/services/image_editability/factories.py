...
from .extractors import (
    ...
    PaddleOCRElementExtractor,
    PaddleAccurateOCRElementExtractor,
    ExtractorRegistry,
)
...
class ExtractorFactory:
    ...
    def create_extractor_registry(
        ...
        # Try to create PaddleOCR extractors (local, no API key needed)
        paddle_ocr_extractor = None
        paddle_accurate_ocr_extractor = None
        try:
            ...
            paddle_table_provider = create_paddle_table_ocr_provider()
            if paddle_table_provider:
                paddle_ocr_extractor = PaddleOCRElementExtractor(paddle_table_provider)
                logger.info("PaddleOCR table extractor created")
            paddle_provider = create_paddle_ocr_provider()
            if paddle_provider:
                paddle_accurate_ocr_extractor = PaddleAccurateOCRElementExtractor(
                    paddle_provider
                )
                logger.info("PaddleOCR text extractor created")
        except Exception as e:
        ...
        return ExtractorRegistry.create_default(
            ...
            paddle_ocr_extractor=paddle_ocr_extractor,
            paddle_accurate_ocr_extractor=paddle_accurate_ocr_extractor,
        )
    ...
    def create_hybrid_extractor(
        ...
        # Try PaddleOCR first (local, no API key), then Baidu OCR
        ocr_extractor = None
        try:
            from services.ai_providers.ocr import create_paddle_ocr_provider

            paddle_provider = create_paddle_ocr_provider()
            if paddle_provider:
                ocr_extractor = PaddleAccurateOCRElementExtractor(paddle_provider)
                logger.info("PaddleOCR text extractor created for hybrid")
        except Exception as e:
            logger.debug(f"PaddleOCR not available for hybrid: {e}")

        if ocr_extractor is None:
            ocr_extractor = ExtractorFactory.create_baidu_accurate_ocr_extractor(
                baidu_accurate_ocr_provider
            )

        if ocr_extractor is None:
            logger.warning("无法创建OCR提取器，混合提取器创建失败")
            return None

        logger.info("✅ OCR提取器已创建（用于混合提取）")

        return HybridElementExtractor(
            mineru_extractor=mineru_extractor,
            baidu_ocr_extractor=ocr_extractor,
